...
input = open('input_test.txt', 'r').read().strip()
...
positions = [np.array(p) for p in scanners.values()]
result = int(np.max(list(np.abs(p1 - p2).sum(-1) for p1, p2 in itertools.combinations(positions, r=2))))
