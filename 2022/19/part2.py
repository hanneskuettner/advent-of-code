...
optimum_geodes = [ ( t - 1 ) * t // 2 for t in range( 32 + 1 ) ]
...
def sim(blueprint):
    ...
    for target in range(4):
        search(np.array([1, 0, 0, 0]), np.zeros(4, dtype=int), target, 32)
    
    return max_geodes


result = sim(blueprints[0]) * sim(blueprints[1]) * sim(blueprints[2])
